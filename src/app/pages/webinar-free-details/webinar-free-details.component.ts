...
@Component({
  selector: "app-webinar-free-details",
  templateUrl: "./webinar-free-details.component.html",
  styleUrls: ["./webinar-free-details.component.css"],
})
export class WebinarFreeDetailsComponent implements OnInit {
  webinar: any = null;
  ...
  redirectToWebinarRegistration() {
    ...
    this.router.navigate(['/auth/register', webinarType, webinarId], { queryParams: { isFree: 'true' } });
  }
  ...
  subscribeToWebinar() {
    ...
    // For free version, we'll redirect to registration directly without payment checks
  ...
  getDisplayFaqs(): { q: string; a: string; }[] {
    const faqs = this.fallbackData?.faqs || [];
    return faqs.filter(f => !f.q.toLowerCase().includes('priced at') && !f.q.includes('99'));
  }
