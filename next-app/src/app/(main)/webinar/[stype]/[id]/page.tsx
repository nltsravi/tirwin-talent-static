...
export default function WebinarDetailsPage() {
  ...
  useEffect(() => {
    ...
    if (webinarId) {
      fetchWebinarDetails(webinarId);
    } else {
      setErrorMessage("Webinar ID is missing");
      setIsLoading(false);
    }
  ...
  const getDisplayPrice = () => {
    const price = parseFloat(webinar?.price || "0");
  ...
  const handleRegisterAction = async () => {
    ...
    if (!isLoggedIn) {
      ...
      router.push(`/auth/register/\${pageType}/\${webinarId}`);
      return;
    }

    // In Angular app, this pushes to the register webinar flow or directly subscribes.
    // For now we push to the Next.js register page with the right IDs. 
    // Wait, the new logic uses `/auth/register/[webinarType]/[id]` directly.
    router.push(`/auth/register/\${pageType}/\${webinarId}`);
  };
    ...
    <div className="inspiro-theme min-vh-100" style={{ paddingTop: "80px" }}>
      ...
      >
        <div className="container text-left">
          <span className="badge badge-no-outline me-2">{webinar?.category?.name}</span>
          <h1 className="display-4 fw-bold">{webinar?.title}</h1>
        </div>
      </section>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              ...
              <div className="modal-body">
                <p>Are you sure you want to register for &quot;{webinar?.title}&quot;?</p>
              </div>
